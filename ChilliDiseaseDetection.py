...
def cnnModel():
    ...
    if os.path.exists('model/model.json'):
        ...
        #defining model with 32, 32 and 256 and due to system memory limit we have restrict image size to 64 X 64 
        model = Sequential() #resnet transfer learning code here
        model.add(Convolution2D(32, 3, 3, input_shape = (64, 64, 3), activation = 'relu'))#define CNN layer with image input size as 64 X 64 with 3 RGB colours
        model.add(MaxPooling2D(pool_size = (2, 2)))#max pooling layer to collect filter data
        model.add(Convolution2D(32, 3, 3, activation = 'relu'))#defining another layer to further filter images
        model.add(MaxPooling2D(pool_size = (2, 2))) #max pooling layer to collect filter data
        model.add(Flatten())#convert images from 3 dimension to 1 dimensional array
        model.add(Dense(output_dim = 256, activation = 'relu'))#defining output layer
        model.add(Dense(output_dim = 2, activation = 'softmax'))#this output layer will predict 1 disease from 2 classes as health and disease
        model.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy'])#compile cnn model
        print(model.summary())
        hist = model.fit(X, Y, batch_size=16, epochs=10, validation_split=0.2, shuffle=True, verbose=2)#build CNN model with given X and Y images
...
def predict():
    ...
    cv2.putText(img, 'Chilli Disease Recognize as : '+plants[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 255, 0), 2)
    cv2.imshow('Chilli Disease Recognize as : '+plants[predict], img)
    cv2.waitKey(0)
